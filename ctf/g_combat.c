...
void T_Damage (edict_t *targ, edict_t *inflictor, edict_t *attacker, vec3_t dir, vec3_t point, vec3_t normal, int damage, int knockback, int dflags, int mod, int HE)
{
	gclient_t	*client;
	int			take;
	int			save;
	...
	while ((ent = findradius(ent, inflictor->s.origin, radius)) != NULL)
	{
		...
		if (points > 0)
		{
			if (CanDamage (ent, inflictor))
			{
				...
				T_Damage (ent, inflictor, attacker, dir, inflictor->s.origin, vec3_origin, (int)points, (int)points, DAMAGE_RADIUS, mod, 0);
			}
		}
	}
}
